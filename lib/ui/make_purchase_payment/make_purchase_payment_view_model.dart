...
class MakePurchasePaymentViewModel extends FormViewModel {
  ...
  Future<bool> makePurchasePayment(String purchaseId) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String businessIdValue = prefs.getString('businessId') ?? '';

    final bool isPayed = await _purchaseService.makePurchasePayment(
        purchaseId: purchaseId,
        businessId: businessIdValue,
        reference: paymentReferenceValue ?? '',
        description: paymentDescriptionValue ?? '',
        total: double.parse(paymentTotalValue ?? ''),
